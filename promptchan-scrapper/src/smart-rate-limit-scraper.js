...
class SmartRateLimitScraper {
  constructor() {
    ...
    this.timeoutMs = 180000; // 3 minutes timeout
    this.downloadDelay = 5000; // 5 seconds between downloads
  ...
  async init() {
    console.log(chalk.blue(`🚀 Initializing Smart Rate Limit Scraper (Session ${this.sessionCount + 1})...`));
    ...
    try {
      ...
      if (this.sessionCount === 1) {
        ...
        await new Promise((resolve) => {
          setTimeout(() => {
            console.log(chalk.blue('🔄 Starting smart rate limit video hunting...'));
            resolve();
          }, 30000);
        });
      }
      ...
      await this.page.goto(this.exploreUrl, { waitUntil: 'networkidle0', timeout: 60000 });
      await this.page.waitForTimeout(5000);
          ...
          if (this.videoUrls.size > 0) {
            ...
            console.log(chalk.blue('📥 Starting SMART RATE LIMIT video downloads...'));
            await this.downloadAllVideosWithSmartRateLimit();
        ...
        if (currentValidVideos < this.targetVideos && this.sessionCount < this.maxSessions) {
          console.log(chalk.yellow('⏳ Waiting 30 seconds before next session...'));
      ...
      console.error(chalk.red('❌ Smart rate limit scraping failed:'), error.message);
    }
  }
  ...
  async downloadAllVideosWithSmartRateLimit() {
    const videoArray = Array.from(this.videoUrls);
    console.log(chalk.blue(`📥 Downloading ${videoArray.length} videos with smart rate limiting...`));
    ...
    for (let i = 0; i < videoArray.length; i++) {
      ...
      while (!success && attempts < this.maxRetries) {
        ...
        try {
          ...
          if (attempts === this.maxRetries) {
            ...
            // Smart backoff: longer wait for rate limit errors
            const isRateLimit = error.message.includes('429') || error.message.includes('rate limit');
            const waitTime = isRateLimit ? 
              Math.min(30000 * Math.pow(2, attempts - 1), 300000) : // Up to 5 minutes for rate limits
              Math.min(5000 * Math.pow(2, attempts - 1), 60000); // Up to 1 minute for other errors
      ...
      // Smart delay between downloads based on success/failure
      const delay = success ? this.downloadDelay : this.retryDelay;
      if (i < videoArray.length - 1) {
        console.log(chalk.cyan(`⏳ Waiting ${delay}ms before next download...`));
  ...
  async saveVideoMetadata() {
    ...
    const filename = `promptchan_smart_rate_limit_${timestamp}.json`;
  ...
  async saveFailedVideos() {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `failed_videos_smart_${timestamp}.json`;
    const filepath = path.join(this.metadataDir, filename);
...
export default SmartRateLimitScraper;

...
class ProxyVideoScraper {
  constructor() {
    ...
    this.batchSize = 1; // Download one at a time
    ...
    this.proxies = [
      ...
      // Example: 'http://user:pass@123.456.789.0:8080'
      // For now, we'll use some free proxies (these may not work, replace with working ones)
      'http://8.210.83.33:80',
  ...
  async init() {
    console.log(chalk.blue('🚀 Initializing Proxy Video Scraper...'));
    console.log(chalk.yellow(`🎯 Target: ${this.targetVideos} VALID VIDEOS`));
    console.log(chalk.cyan(`🌐 Using ${this.proxies.length} proxies for rotation`));
    ...
    try {
      ...
      if (proxy) {
        ...
        console.log(chalk.yellow('⚠️  No proxy configured, using direct connection'));
      }
  ...
  async run() {
    try {
      ...
      await new Promise((resolve) => {
        setTimeout(() => {
          console.log(chalk.blue('🔄 Starting proxy video hunting...'));
          resolve();
        }, 30000);
      });
      ...
      if (this.videoUrls.size > 0) {
        ...
        console.log(chalk.blue('📥 Starting PROXY video downloads with retry logic...'));
        await this.downloadAllVideosWithProxy();
        ...
        if (finalValidVideos < this.targetVideos) {
          ...
          console.log(chalk.blue('🔄 Retrying failed downloads with different proxies...'));
          await this.retryFailedDownloadsWithProxy();
        }
      } else {
  ...
  async downloadAllVideosWithProxy() {
    const videoArray = Array.from(this.videoUrls);
    console.log(chalk.blue(`📥 Downloading ${videoArray.length} videos with proxy rotation...`));
      ...
      while (!success && attempts < this.maxRetries) {
        ...
        try {
          ...
          if (attempts === this.maxRetries) {
            ...
            console.log(chalk.yellow(`⏳ Waiting ${waitTime}ms before retry with different proxy...`));
  ...
  async retryFailedDownloadsWithProxy() {
    if (this.failedVideos.length === 0) {
      console.log(chalk.green('✅ No failed videos to retry!'));
      return;
    }

    console.log(chalk.blue(`🔄 Retrying ${this.failedVideos.length} failed downloads with different proxies...`));
  ...
  async saveVideoMetadata() {
    ...
    const filename = `promptchan_proxy_videos_${timestamp}.json`;
  ...
  async saveFailedVideos() {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `failed_videos_proxy_${timestamp}.json`;
    const filepath = path.join(this.metadataDir, filename);
...
export default ProxyVideoScraper;
